...
__global__ void sieve_of_eratosthenes(bool* is_prime, int chunk_size, int threads_number, int N)
{
  ...
  int end = (threadIdx.x == threads_number - 1) ? N : (threadIdx.x + 1) * chunk_size - 1;
...
void run_threads(int threads_number, int N)
{
  ...
  sieve_of_eratosthenes<<<1, threads_number>>>(is_prime, chunk_size, threads_number, N);
